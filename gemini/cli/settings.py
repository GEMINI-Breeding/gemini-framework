...
def set_local(ctx: GEMINISettingsContext, enable: bool):
    ...
    if enable is None:
        ...
        return
    if settings.GEMINI_LOCAL == enable:
        click.echo(click.style(f"Local mode is already {'enabled' if enable else 'disabled'}.", fg="yellow"))
        return
    ctx.manager.set_setting("GEMINI_LOCAL", enable)
    click.echo(click.style(f"Local mode set to: {enable}", fg="green"))
...
def set_domain(ctx: GEMINISettingsContext, domain: str):
    ...
    if domain is None:
        ...
        return
    current_settings = ctx.manager.get_settings()
    if current_settings.GEMINI_PUBLIC_DOMAIN == domain:
