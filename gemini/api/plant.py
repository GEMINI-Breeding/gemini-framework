...
class Plant(APIBase):
    ...
    @classmethod
    def search(cls, **search_parameters) -> List["Plant"]:
        try:
            plants = PlantViewModel.search(**search_parameters)
            plants = [cls.model_validate(plant) for plant in plants]
            return plants if plants else None
        except Exception as e:
            raise e
    ...
    def update(self, **update_parameters) -> "Plant":
        try:
            current_id = self.id
            plant = PlantModel.get(current_id)
            plant = PlantModel.update(plant, **update_parameters)
